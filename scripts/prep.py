...
@click.argument('output_path', required=True, type=click.Path(exists=True))
@click.option('--fhir_version', required=False, default='R5', help='FHIR version for output files (R4 or R5) default:R5')
def part_of(input_path, output_path, fhir_version):
    """Apply partOf extension to all resources.

    \b
    INPUT_PATH  the input META directory
    ...
    # Apply partOf to all resources
    for resource in pathlib.Path(input_path).glob('*.ndjson'):
        if 'ResearchStudy' in resource.name:
            continue
        with open(resource, 'r') as infile, open(pathlib.Path(output_path) / resource.name, 'w') as outfile:
            for line in infile:
                resource = json.loads(line.strip())
                _ = apply_part_of(resource, research_study_id=research_study['id'])
                outfile.write(json.dumps(_) + '\n')
    ...
    def emit(self, resource):
        ...
        if resource:
            if resource['resourceType'] not in self._files:
                self._files[resource['resourceType']] = open(self._output_path / f"{resource['resourceType']}.ndjson", 'w')
...
def validate(resource, fhir_version, *args, **kwargs):
    ...
    try:
        ...
        click.echo(f"Validation error: {fhir_version} {klass} {e}\n{json.dumps(resource, indent=2)}")
        exit(1)
...
def reseed(resource, seed, *args, **kwargs):
    ...
    def callback(reference):
        ...
        return f"{resource_type}/{str(uuid.uuid5(uuid.NAMESPACE_DNS, resource_id + seed))}"
...
@click.option('--fhir_version', required=False, default='R5', help='FHIR version for output files (R4 or R5) default:R5')
@click.option('--seed', required=False, default=None, help='Reseed all references with the new seed')
@click.option('--transformers', required=False, default='assay,part-of,validate', help='Transformation steps.  Known steps are: [assay,part-of,r4,validate,validate_references, reseed]')
def prep(input_path, output_path, fhir_version, transformers, seed):
    """Run a set of transformations on the input META directory.

    \b
    INPUT_PATH META directory containing the DocumentReference, Group, and Specimen NDJSON files
    OUTPUT_PATH the output META directory
    """
    ...
    if not set(list(transformer_map.keys())) == set(transformers):
        ...
        if unknown_transformers != {'assay'}:
            click.secho(f"Unknown transformers: {unknown_transformers}", file=sys.stderr, fg='red')
            exit(1)
    if 'reseed' in transformers and not seed:
        click.secho(f"reseed specified, please specify --seed", file=sys.stderr, fg='red')
        exit(1)

    emitted = []
    click.echo(f"Transformers: {transformers}", file=sys.stderr)
    with Halo(text="Processing", spinner="line", placement="right", color="white", stream=sys.stderr) as spinner:
        ...
        if 'assay' in transformers:
            for resource in create_assays(fhir_version, input_path):
                for transformer in transformer_map.values():
                    resource = transformer(resource=resource, research_study_id=research_study_id, fhir_version=fhir_version, seed=seed)
                if not resource:
                    continue
                emitters.emit(resource)
                if last_resource_type and last_resource_type != resource['resourceType']:
        ...
        for resource in pathlib.Path(input_path).glob('*.ndjson'):
            ...
            with open(resource, 'r') as infile:
                for line in infile:
                    resource = json.loads(line.strip())
                    for transformer in transformer_map.values():
                        resource = transformer(resource=resource, research_study_id=research_study_id,
                                               fhir_version=fhir_version, seed=seed)
                    if not resource:
                        continue
                    emitters.emit(resource)
                    if last_resource_type and last_resource_type != resource['resourceType']:
...
def create_assays(fhir_version, input_path) -> Generator[dict, None, None]:
    ...
    group = pathlib.Path(input_path) / 'Group.ndjson'
    ...
    for doc in document_references:
        if doc['subject']['reference'].startswith('Specimen/'):
            ...
            assert doc['subject']['reference'].startswith(
                'Patient/'), f"Document subject is not a patient: {doc['subject']['reference']}"
            assays.append(assay_dict)
    docs_with_non_patient_subject = [(doc['id'], doc['subject']['reference']) for doc in document_references if
                                     not doc['subject']['reference'].startswith('Patient/')]
    assert len(docs_with_non_patient_subject) == len(
        groups), f"Documents have groups with non-patient subject: {docs_with_non_patient_subject}"
...
def create_assay_refactor_docs(assay_id: str, patient_reference: str, specimen_references: list[str], assay_documents: list[dict], fhir_version='R4') -> dict:
    """
    ...
    for doc in assay_documents:
        ...
        if fhir_version != 'R4':
            ...
            context['related'].extend(specimen_references)
